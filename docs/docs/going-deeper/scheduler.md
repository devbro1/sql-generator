...
```ts
scheduler().setAtomicLockHandler(async (name, ttl) => {
  return await cache().getLock(name, ttl);
});
...
See [Start Your Server](../tutorial-basics/start-your-server) for more details on the `--cron-names` filter.
